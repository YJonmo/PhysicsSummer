...
if __name__ == "__main__":
    ...
    if (Spec1.Error == 1) & (DAQ1.Error == 1) & (Power_meter.Error == 1):
        ...
        if (DAQ1.Error == 0):
            ...
            ScansPerRead = int(round(DAQ_SamplingRate*DurationOfReading,-2))
        ...
        if (DAQ1.Error == 0):
            ...
            plt.title('Original vs Filtered Temperature')
            plt.xlabel('Time (s)')
            plt.ylabel('Temperature ($^\circ$C)')
            plt.ylim([0,50])
            plt.legend(['Original Temperature','Filtered Temperature'])
            plt.show()
